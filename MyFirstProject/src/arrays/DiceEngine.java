...
public class DiceEngine {
	...
	public static void main(String[] args) {
		...
		String[][] stringArray = new String[rolls][2];

		// Prints out numbers
		for (int pass = 0; pass < rolls; pass++) {
			ArrayList<int[]> temp = new ArrayList(Arrays.asList(rollArray));
			int total = 1;
			for (int j = pass + 1; j < rolls-1; j++) {
				
				System.out.println(temp.get(pass).equals(temp.get(j)));
				if (Arrays.toString(temp.get(pass)).equals(Arrays.toString(temp.get(j)))) {
			...
			stringArray[pass][1] = Arrays.toString(rollArray[pass]);
